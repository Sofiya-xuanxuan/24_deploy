...
function generateDisplayName(sourceComponent, injectNames) {
  var displayName = 'inject-' + (sourceComponent.displayName || sourceComponent.name || sourceComponent.constructor && sourceComponent.constructor.name || 'Unknown');
...
function observer(Component, renderMedthod) {
  ...
  var originRender = target[renderMedthod];
  ...
  target[renderMedthod] = function () {
    ...
    if (this._reaction && this._reaction instanceof mobx.Reaction) {
      this._reaction.track(function () {
        try {
          result = mobx._allowStateChanges(false, function () {
            return originRender.call(_this2, null, null, args[2]);
          });
        } catch (e) {
          exception = e;
        }
